...
class Block():
    ...
    def __init__(self, parent_screen, number):
        ...
        self.image_2 = pygame.image.load('block_2.png')
        self.x = [random.randint(1, 35) * SIZE + 8]
        self.y = [random.randint(4, 27) * SIZE + 28]
...
####################################################################### Conditions to first spawn an object########################################################################################
        ...
        if self.snake.counter == 1 and self.TranslucentPotion.x > 2800:
...
################################################################# ALL THE CONDITIONS FOR THE DIFFERENT COLLISIONS ##################################################################################
        ...
        for i in range(0, self.block.number):
            if self.is_collision(self.snake.x[0], self.snake.y[0], self.block.x[i], self.block.y[i]) and self.snake.translucent == False:
                self.play_sound('crash')
                #self.play_sound('game over')
                raise "Collision Occurred"

            
        #if self.is_collision(self.snake.x[-1], self.snake.y[-1], self.block.x[i], self.block.y[i]) and self.snake.translucent == True:
            #self.collision = True
        #else:
            #self.collision == False
...
################################################################# Different Times for Different Potions###########################################################################################
        ...
        if self.snake.translucent:
            ...
            while j <= self.snake.length:
                for i in range(0, self.block.number):
                    if self.is_collision(self.snake.x[j], self.snake.y[j], self.block.x[i],
                                        self.block.y[i]):
                        self.block.draw(i)
        ...
        #if (self.translucent == True) and (self.collision == True) and abs(self.snake[-1] - self.block[passed_blocks[0]]) > 20:
            #for i in range(len(passed_blocks)):
                #self.block.draw(i)
                #self.translucent == False
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    ...
                    if not pause:
                        if ((not self.snake.reversed_movement) and event.key == K_LEFT) or (self.snake.reversed_movement and event.key == K_RIGHT):
                            if not (self.snake.x[1] < self.snake.x[0]):
                                self.snake.move_left()

                        elif ((not self.snake.reversed_movement) and event.key == K_RIGHT) or (self.snake.reversed_movement and event.key == K_LEFT):
                            if not (self.snake.x[1] > self.snake.x[0]):
                                self.snake.move_right()

                        elif ((not self.snake.reversed_movement) and event.key == K_UP) or (self.snake.reversed_movement and event.key == K_DOWN):
                            if not (self.snake.y[1] < self.snake.y[0]):
                                self.snake.move_up()

                        elif ((not self.snake.reversed_movement) and event.key == K_DOWN) or (self.snake.reversed_movement and event.key == K_UP):
                            if not (self.snake.y[1] > self.snake.y[0]):
